  ...
  <div className="space-y-12 text-gray-700 dark:text-gray-300 my-16">
    ...
    <section className="bg-(--card) p-6 md:p-8 rounded-(--surface-radius) border border-(--border) shadow-theme-sm backdrop-blur-sm theme-transition">
      ...
      <p className="leading-relaxed mb-6 text-base md:text-lg text-(--textBody)">
        Transform your PDF documents into high-quality JPEG images effortlessly.
        Whether you need to extract individual pages or convert an entire
        document for{" "}
        <strong>social media, presentations, or school projects</strong>, our
        tool ensures your files stay sharp and clear. This is a 100%{" "}
    ...
    <section className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {[
        {
          step: "Select PDF",
          desc: "Choose your PDF file. It stays in your browser for 100% privacy.",
          icon: FileText,
          color: "text-purple-600 bg-purple-50 dark:bg-purple-500/10",
        },
        {
          step: "Instant Extract",
          desc: "Our engine renders each page into a crisp JPG at 150-300 DPI.",
          icon: Zap,
          color: "text-amber-600 bg-amber-50 dark:bg-amber-500/10",
        },
        {
          step: "Download ZIP",
          desc: "Save individual pages or download everything in one neat ZIP file.",
          icon: Download,
        ...
        {[
          {
            ...
            a: "No. We use high-resolution rendering (150 DPI baseline) which ensures that even small text in your PDF remains readable after converting to JPG.",
          },
          {
            q: "Can I convert multiple pages at once?",
            a: "Yes. Our tool automatically processes every page of your PDF and allows you to download them all as a single ZIP folder.",
          },
          {
            q: "Is it safe for sensitive government documents?",
            a: "Absolutely. Since the conversion happens locally on your device, your Aadhaar, PAN, or other certificates are never uploaded to the internet.",
          },
        ].map((faq, i) => (
          <div
            key={i}
...
const PdfToJpgContent: React.FC = () => {
  ...
  const downloadAll = async () => {
    ...
    try {
      const fileSaver = await safeLoadLibrary<{
        saveAs: (b: Blob | string, n: string) => void;
      }>(() => import("file-saver"), "file-saver");
      if (pages.length === 1) {
        fileSaver.saveAs(
          pages[0],
  ...
  transition-all text-center group"
          >
            ...
            <h3 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white px-6">
              Select PDF Document
            </h3>
            <p className="text-gray-500 text-xs mt-2">
              No uploads • 100% Client-Side Processing
            </p>
            <input
              ref={fileInputRef}
              type="file"
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/60 p-4 md:p-5 rounded-2xl md:rounded-3xl border border-(--border)">
              <div className="flex items-center gap-3 md:gap-4 text-left min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-gray-900 dark:text-white font-bold truncate max-w-35 md:max-w-xs">
                    {file.name}
                  </p>
              ...
              <button
                onClick={reset}
                className="p-2 md:p-3 rounded-xl hover:bg-red-500/10 transition text-red-500 shrink-0"
              >
            ...
            {isProcessing && (
              <div className="py-10 md:py-12 text-center space-y-6 md:space-y-8">
                ...
                <h3 className="text-lg md:text-2xl font-bold text-gray-900 dark:text-white tracking-tight">
                  Rendering Pages... {progress}%
                </h3>
                <div className="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden max-w-60 md:max-w-sm mx-auto shadow-inner">
                  <div
              ...
              <div className="space-y-6 md:space-y-8">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-green-500/5 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-green-500/10">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-sm md:text-base text-gray-900 dark:text-white tracking-tight">
                      {pages.length} Pages Extracted
                    </span>
                  </div>
                  <button
                    onClick={downloadAll}
                    className="w-full sm:w-auto px-6 py-3 md:px-8 md:py-4 bg-green-600 hover:bg-green-500 text-white rounded-xl md:rounded-2xl font-bold shadow-lg shadow-green-500/10 transition-all flex items-center justify-center gap-2 md:gap-3"
                  >
                    <Download className="w-4 h-4 md:w-5 md:h-5" /> Download All
                    (ZIP)
                  </button>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4 max-h-100 md:max-h-125 overflow-y-auto p-3 md:p-4 bg-gray-50 dark:bg-gray-900/40 rounded-2xl md:rounded-3xl border border-(--border) custom-scrollbar">
                  {pages.map((img, i) => (
                    <div
                      key={i}
                      className="group relative rounded-xl md:rounded-2xl overflow-hidden border border-(--border) shadow-sm hover:shadow-md transition-all"
                    >
                      <img
                        src={img}
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-[2px]">
                        <a
                          ...
                          className="px-3 py-1.5 bg-white text-black text-[10px] md:text-xs font-bold rounded-lg"
                        >
                          Download JPG
                        </a>
                      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mt-10 md:mt-12">
        <div className="flex items-center justify-center gap-3 p-4 md:p-5 bg-white/50 dark:bg-gray-950/10 rounded-2xl md:rounded-3xl border border-(--border) shadow-theme-sm">
          <Lock className="w-4 h-4 md:w-5 md:h-5 text-purple-500 md:font-semibold" />{" "}
          <span className="text-[10px] md:text-xs font-bold text-gray-500 dark:text-gray-100 uppercase tracking-wider">
            100% Private Engine
          </span>
        </div>
        <div className="flex items-center justify-center gap-3 p-4 md:p-5 bg-white/50 dark:bg-gray-950/10 rounded-2xl md:rounded-3xl border border-(--border) shadow-theme-sm">
          <Smartphone className="w-4 h-4 md:w-5 md:h-5 text-purple-500 md:font-semibold" />{" "}
          <span className="text-[10px] md:text-xs font-bold text-gray-500 dark:text-gray-100 uppercase tracking-wider">
            Responsive UI
          </span>
        </div>
        <div className="flex items-center justify-center gap-3 p-4 md:p-5 bg-white/50 dark:bg-gray-950/10  rounded-2xl md:rounded-3xl border border-(--border) shadow-theme-sm">
          <ShieldCheck className="w-4 h-4 md:w-5 md:h-5 text-purple-500 md:font-semibold" />{" "}
          <span className="text-[10px] md:text-xs font-bold text-gray-500 dark:text-gray-100 uppercase tracking-wider">
            SSL Secure Conversion
          </span>
        </div>
      </div>
      <UniqueSEOContent />
    ...
    <ToolLandingPage
      ...
      heading={
        <span className="text-gray-900 dark:text-white font-bold">
          PDF to{" "}
          <span className="text-purple-600 dark:text-purple-500 italic">
            JPG
          </span>
        </span>
      }
