  ...
  <div className="space-y-12 text-gray-700 dark:text-gray-300 mt-16 max-w-5xl mx-auto">
    ...
    <section className="bg-(--card) p-8 md:p-12 rounded-(--surface-radius) border border-(--border) shadow-theme backdrop-blur-sm theme-transition relative overflow-hidden">
      ...
      <p className="leading-relaxed mb-8 text-lg text-(--textBody) max-w-3xl font-medium">
        ...
        <strong>"how to merge pdf documents in preview"</strong> or struggling
        with complex software? Our online tool is the easiest way to{" "}
        <strong>merge pdf documents on acrobat</strong> alternative platforms.
        Whether you need to <strong>merge pdf on mac</strong> or looking for a{" "}
    ...
    <section className="space-y-10">
      <div className="text-center space-y-2">
        <h3 className="text-3xl font-black text-(--textHeading) tracking-tighter italic uppercase">
          Help & FAQ
        </h3>
        <p className="text-(--textMuted) font-bold italic uppercase text-xs tracking-widest">
...
const MergePdfContent: React.FC = () => {
  ...
  const mergePdfs = async () => {
    ...
    try {
      const { PDFDocument } = await safeLoadLibrary(
        () => import("pdf-lib"),
        "pdf-lib",
      );
      ...
      const fileSaverModule = await safeLoadLibrary<any>(
        () => import("file-saver"),
        "file-saver",
      );
      const saveAs =
                      ...
                      {(provided: DraggableProvided) => (
                        ...
                        >
                          <div
                            ...
                            className="text-gray-600 cursor-grab hover:text-blue-400 p-1 italic"
                          >
                          ...
                          <div className="grow min-w-0">
                            ...
                            <p className="text-xs text-(--textMuted)">
                              {(file.file.size / 1024).toFixed(0)} KB • Order:{" "}
                              {index + 1}
                            </p>
                          </div>
                          <button
...
const MergePdf: React.FC<{ routeKey?: string }> = ({
  routeKey = "/merge-pdf",
}) => (
