...
const ObedinitPdfContent: React.FC = () => {
  ...
  const mergePdfs = async () => {
    if (files.length < 2) {
      alert("Пожалуйста, выберите как минимум 2 PDF файла для объединения.");
      return;
    }
    ...
    try {
      ...
      alert("Не удалось объединить PDF. Один из файлов может быть поврежден.");
    } finally {
    ...
    <div className="w-full">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8 text-center border-2 border-dashed border-gray-700 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all cursor-pointer relative group rounded-3xl p-10 bg-(--bg2)">
          ...
          <h3 className="text-xl font-bold text-white mb-1">Добавить PDF файлы</h3>
          <p className="text-sm text-gray-500">Объединяйте справки, сертификаты или любые документы</p>
        </div>
                      ...
                      {(provided: DraggableProvided) => (
                        ...
                        >
                          ...
                          <div className="grow min-w-0">
                            ...
                            <p className="text-xs text-gray-500">{(file.file.size / 1024).toFixed(0)} КБ • Порядок: {index + 1}</p>
                          </div>
...
export default function ObedinitPdf() {
  return (
    <ToolErrorBoundary toolName="Объединить PDF">
      <ToolLandingPage
        routeKey="/ru/obedinit-pdf"
        heading={<>Объединить <span className="text-blue-400">PDF</span> файлы</>}
        tagline="Склеивайте документы легко • Приватно • Быстро"
      >
        <ObedinitPdfContent />
      </ToolLandingPage>
    </ToolErrorBoundary>
